...
const Finance = () => {
  ...
  return (
    <div className='grid  gap-5 px-16'>
      <h1 className='text-center font-bold font-[Poppins] text-3xl dark:text-white'>
        Expense Tracker
      </h1>
      <div className='flex flex-row gap-8 p-8'>
        {/* income */}
        <div className='flex-[2] p-8 w-[20%] bg-white rounded-md justify-center items-center dark:bg-slate-700'>
          <h2 className='text-center pb-3 font-semibold round dark:text-white'>
            Income
          </h2>
          <Chart />
        </div>

        <div className='flex-[3] flex flex-col items-center justify-center w-96 rounded-2xl shadow-2xl bg-white dark:bg-slate-700'>
          {choice === "income" && <Income setChoice={setChoice} />}
          {choice === "expense" && <Expense setChoice={setChoice} />}

          {choice === "" && (
            <>
              <h2 className='text-center mb-36 font-semibold dark:text-white'>
